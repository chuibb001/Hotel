...
-(void)reset
{
    ...
    [gregorian components:(NSYearCalendarUnit | NSMonthCalendarUnit |
                           NSDayCalendarUnit) fromDate: _cDate];
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touchPoint.y > kVRGCalendarViewTopBarHeight) {
        ...
        //NSLog(@"%d",date);
        if(date<[_sDate day] && [currentMonth month]==[_sDate month])
            ;
        else
    ...
    if (CGRectContainsPoint(rectArrowLeft, touchPoint) && [_sDate month]!=[currentMonth month])
    {
        [self showPreviousMonth];
    }
    else if (CGRectContainsPoint(rectArrowRight, touchPoint)&& [_sDate month]+3!=[currentMonth month])
    {
        [self showNextMonth];
    }
...
- (void)drawRect:(CGRect)rect
{
    ...
    // 貌似时间段限制为90天
    //Arrow Left
    if([_sDate month]!=[currentMonth month])  // 正在浏览的月份和s
    {
    ...
    //Arrow right
    if([_sDate month]+3!=[currentMonth month])
    {
        CGContextBeginPath(context);
    ...
    Boolean isStartMonth=NO;  // 判断startMonth 是否等于当前浏览的 currentMonth
    
    if ([_sDate month] == [currentMonth month] && [_sDate year] == [currentMonth year]) {
        isStartMonth=YES;
    }
    
    if ([_cDate month] == [currentMonth month] && [_cDate year] == [currentMonth year]) {
        todayBlock = [_cDate day] + firstWeekDay - 1;
        //NSLog(@"%@",_cDate);
    }
    for (int i=0; i<numBlocks; i++) {
        int targetDate = i;  // Day数字
        ...
        { // 上个月的day
            ...
            NSString *hex = (isStartMonth) ? @"aaaaaa" : @"0x383838";
            CGContextSetFillColorWithColor(context,
        ...
        { // 下个月的day
            ...
            /*----------modify-----------
             如果小于开始日期，也画浅色
             ----------------------------*/
            Boolean isLessthanStart=NO;
            if(targetDate <[_sDate day])
            {
                isLessthanStart=YES;
            }
            //NSString *hex = (isSelectedDatePreviousMonth || isSelectedDateNextMonth || isLessthanStart) ? @"0xaaaaaa" : @"0x383838";
            NSString *hex = (isLessthanStart && isStartMonth) ? @"aaaaaa" : @"0x383838";
            CGContextSetFillColorWithColor(context,
                                           [UIColor colorWithHexString:hex].CGColor);
        }
        ...
        } else if (todayBlock==i) // startDate 块
        {
            CGRect rectangleGrid = CGRectMake(targetX,targetY,kVRGCalendarViewDayWidth+2,kVRGCalendarViewDayHeight+2);
            CGContextAddRect(context, rectangleGrid);
