...
//        frame.origin.y += self.frame.size.height + [[UIApplication sharedApplication] statusBarFrame].size.height;
      //  self.table = [[SIMenuTable alloc] initWithFrame:self.menuContainer.frame items:self.items];
         self.table = [[SIMenuTable alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2-70, 0, 140, 220) items:self.items];
//        NSLog(@"%d %d %d %d",self.menuContainer.frame.origin.x,self.menuContainer.frame.origin.y,self.menuContainer.frame.size.width,self.menuContainer.frame.size.height);
